...
* **Rename**: opens a form for changing the channel's name.
* **Chat**: opens or closes the channel's text chat. Closed, only the owner and the allowed members write in it; reopened, the chat goes back to the access its category grants.
...
* **Transfer**: hands ownership of the channel to another member present in the voice channel. The new owner receives the powers granted by the server configuration.
* **Claim**: lets someone claim ownership of the channel if the original creator has left and the channel is orphaned.
* **Reserve**: limits access to a specific Discord role picked from a dropdown.
