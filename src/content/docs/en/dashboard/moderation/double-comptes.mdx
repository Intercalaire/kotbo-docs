...
Security is managed from the **Security → Alt accounts** section of your dashboard: [Open the Alt accounts module ↗](https://dash.kotbo.fr/security/accounts).
...
* **Link found elsewhere**: if the staff of another server on the instance already linked this account to another, Kotbo takes their verdict as a detection signal.

The **Links on other servers** block in the member file details those suggestions: the account concerned, how many servers already carry the link, and whether it was set there **Auto**matically or **Manual**ly. Each suggestion shows the bonus it adds to the detection score (*+N pts*), which grows with the number of servers involved, and its status here: **Absent here**, **Already linked here**, or the **Link here** button.

:::note[Nothing is linked automatically]
*Nothing is linked automatically: it is up to you to confirm.* A link found elsewhere stays a suggestion until a staff member of this server reproduces it.
:::
* **Make a decision**:
...
:::note[The switch drives the module]
**Security verification** - *Verifies the identity through Discord OAuth* - drives the module of the same name in the [catalogue](/en/dashboard/configuration/modules/): saving it on enables the module, saving it off shuts it down. The tab's settings only appear while it is on.
:::
...
* **Automatic warn expiry (days)**: past this delay a warn is archived automatically. It leaves the active record without being erased. Empty or `0` means *never*.
* **Count archived sanctions**: by default an archived sanction no longer weighs on the warn score or escalation. Tick this to keep it counting - archiving then becomes purely visual.
* Click **Save** in the green bottom bar after every change.

:::note[The link with manual archiving]
These two settings drive the same mechanism as the **Archive** / **Unarchive** bulk actions on the [Sanctions](/en/dashboard/moderation/sanctions/) page.
:::
