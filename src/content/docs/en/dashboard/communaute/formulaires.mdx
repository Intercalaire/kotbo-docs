...
:::note[Access]
This module is available on your dashboard from the **Moderation/Community → Forms** tab: [Manage my forms ↗](https://dash.kotbo.fr/forms).
:::
...
### 1. Header media
* **Banner URL**: the header image shown at the top of the form. Recommended format: PNG/JPEG at 1200x400px.
* **Logo URL**: your server's avatar or logo, shown in a floating frame above the form.

### 2. Colour palette
You can set the public page's colours precisely:
* **Accent colour**: used for primary buttons, focus outlines, ticked boxes and active states.
* **Background colour**: the overall tint of the web page.
* **Card colour**: the colour of the blocks holding the questions.
* **Text colour**: the general colour of labels and descriptions.
...
### 4. Container styles
* **Border radius**: adjust how rounded the question cards are (from 0px for square corners to 24px for a very rounded look).
* **Glassmorphism**: turn this effect on to make the cards semi-transparent with an elegant background blur, which works well over a coloured or gradient background.
