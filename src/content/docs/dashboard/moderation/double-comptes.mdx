...
* **Lien constaté ailleurs** : si le staff d'un autre serveur de l'instance a déjà lié ce compte à un autre, Kotbo reprend son verdict comme signal de détection.

Le bloc **Liens sur d'autres serveurs** du dossier membre détaille ces suggestions : le compte visé, le nombre de serveurs qui portent déjà le lien, et s'il y a été posé en **Auto** ou en **Manuel**. Chaque suggestion indique le bonus apporté à la note de détection (*+N pts*), qui augmente avec le nombre de serveurs concernés, et son état ici : **Absent d'ici**, **Déjà lié ici**, ou le bouton **Lier ici**.

:::note[Rien n'est lié automatiquement]
*Rien n'est lié automatiquement : à vous de valider.* Une liaison constatée ailleurs reste une suggestion tant qu'un membre du staff de ce serveur ne l'a pas reproduite.
:::
...
:::note[L'interrupteur pilote le module]
**Vérification de sécurité** - *Vérifie l'identité via Discord OAuth* - commande le module du même nom dans le [catalogue](/dashboard/configuration/modules/) : l'enregistrer allumé active le module, l'enregistrer éteint le coupe. Les réglages de l'onglet n'apparaissent que lorsqu'il est allumé.
:::
...
* **Expiration automatique des warns (jours)** : au-delà de ce délai, un warn est archivé automatiquement. Il quitte le casier actif sans être effacé. Vide ou `0` signifie *jamais*.
* **Compter les sanctions archivées** : par défaut, une sanction archivée ne pèse plus dans le score de warns ni dans l'escalade. Cochez pour qu'elle continue de compter - l'archivage devient alors purement visuel.
* Cliquez sur **Enregistrer** dans la barre inférieure verte après chaque modification.

:::note[Le lien avec l'archivage manuel]
Ces deux réglages commandent le même mécanisme que les actions groupées **Archiver** / **Désarchiver** de la page [Sanctions](/dashboard/moderation/sanctions/).
:::
